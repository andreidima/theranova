...
<div class="row mb-0 px-3 d-flex border-radius: 0px 0px 40px 40px" id="adaugareRecoltareSange">
    ...
    <div v-if="nrPungi > 0" class="col-lg-8 px-4 py-2 mb-4 mx-auto border border-1 border-warning">
        <div class="row mb-0">
            ...
            <div v-for="i in nrPungi" class="col-lg-3 mb-4 mx-auto">
                <label for="cantitate" class="mb-0 ps-3">Punga @{{ i }}<span class="text-danger">*</span></label>
                    <input
                        type="text"
                        class="form-control bg-white rounded-3 {{ $errors->has('cantitate') ? 'is-invalid' : '' }}"
                        :name="'cantitatiRecoltariSange[' + i + ']'"
                        v-model="cantitatiRecoltariSange[i]">
            </div>
        </div>
    </div>
